...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 云日志管理 <span class="c-gray en">&gt;</span> 日志列表<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div class="text-c">
        <form class="Huiform" method="get" action="cloudlogListByKey" target="_self">
       	 <input type="hidden" value="1"  name="page">	
            <input type="text" class="input-text" style="width:250px" placeholder="设备编号" name="key">
            <button type="submit" class="btn btn-success" name=""><i class="Hui-iconfont">&#xe665;</i> 搜日志</button>
        </form>
    </div>
			 ...
			 <div class="col-6 col-offset-5 printers-page">
		        <div class="page"> 
					...
					<c:if test="${pageBean.page!=1}">
						<span ><a href="cloudlogListByKey?page=1&key=${key}">首页</a></span>    	
						<span ><a href="cloudlogListByKey?page=${pageBean.page-1}&key=${key}">上一页</a></span>
					</c:if>
					<c:if test="${pageBean.page==1}">
						<span >首页</span>
						<span >上一页</span>
		            ...
		            <div class="pageNum">
			           <c:forEach begin="${curPage}" end="${totalPage}" var="i">
						 	...
						 	<c:if test="${i!=pageBean.page}">
						 		<a href="cloudlogListByKey?page=${i }&key=${key}">${i }</a>
						 	</c:if>	
						</c:forEach> 
		            </div>
					 ...
					 <c:if test="${pageBean.page!=pageBean.totalPage}">
						<span ><a href="cloudlogListByKey?page=${pageBean.page+1}&key=${key}">下一页</a></span>
						<span ><a href="cloudlogListByKey?page=${pageBean.totalPage }&key=${key}">尾页</a></span> 
					</c:if>
					<c:if test="${pageBean.page == pageBean.totalPage}">
						<span >下一页</span>
						<span >尾页</span>
...
<script type="text/javascript">
    ...
    function cloudlogInfo(title,url,id,w,h){
        var url=url+'?id='+id;
